...
{
 "use strict";
 var Global,Website,Client,EventTarget,Node,WebSharper,JavaScript,JS,UI,AttrModule,Operators,Pervasives,CodeMirror,Hint,BuiltinFun,WindowOrWorkerGlobalScope,Obj,Doc,AttrProxy,Elt,Client$1,Templates,DomUtility,Attrs,View,DocElemNode,Updates,Collections,Dictionary,Arrays,Array,Object,Snap,Attrs$1,Dyn,Unchecked,SC$1,HashSet,Seq,Enumerator,CharacterData,Docs,SC$2,List,T,DictionaryUtil,Prepare,Slice,KeyCollection,An,Settings,Abbrev,Mailbox,Fresh,SC$3,Strings,Docs$1,RunState,NodeSet,Anims,SC$4,T$1,SC$5,Concurrency,AppendList,HashSetUtil,SC$6,Easing,AsyncBody,SC$7,CT,HashSet$1,Scheduler,CancellationTokenSource,DomNodes,Queue,Error,OperationCanceledException,Lazy,SC$8,LazyExtensionsProxy,LazyRecord,console,IntelliFactory,Runtime,$,Date;
 Global=self;
 ...
 Hint=CodeMirror&&CodeMirror.Hint;
 BuiltinFun=Hint&&Hint.BuiltinFun;
 ...
 {
  var options,r,cm,resultContainer,x,a;
  options=(r={},r.mode="javascript",r.lineNumbers=true,r.extraKeys={
   "Ctrl-Space":function(cm$1)
   {
    CodeMirror.showHint(cm$1,CodeMirror.javascriptHint);
   }
  },r);
  cm=CodeMirror.fromTextArea(self.document.getElementById("editor"),options);
  console.log(cm.charCoords({
   line:1,
   ch:1
  },"page"));
  cm.openDialog("bar:<input/>",function(x$1)
  {
   return console.log(x$1);
  });
  resultContainer=Doc.Element("pre",[AttrModule.Class("cm-s-default")],[]);
  x=Doc.Element("div",[],[Doc.Element("button",[AttrProxy.HandlerImpl("click",function()
  {
   return function()
   {
    return CodeMirror.runMode(cm.getValue(),"javascript",resultContainer.elt);
   };
  })],[Doc.TextNode("Run mode")]),Doc.Element("button",[AttrProxy.HandlerImpl("click",function()
  {
   return function()
   {
    return resultContainer.Clear();
   };
  })],[Doc.TextNode("Clear")]),resultContainer]);
 ...
 Docs.CreateDelimitedElemNode=function(ldelim,rdelim,attr,children)
 {
  var el,attr$1;
  el=ldelim.parentNode;
  Docs.LinkPrevElement(rdelim,children);
  attr$1=Attrs.Insert(el,attr);
  return DocElemNode.New(attr$1,children,{
   $:1,
   $0:[ldelim,rdelim]
  },el,Fresh.Int(),Runtime.GetOptional(attr$1.OnAfterRender));
 };
